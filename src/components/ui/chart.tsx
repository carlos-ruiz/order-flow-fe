...
const ChartStyle = ({ id, config }: { id: string; config: ChartConfig }) => {
  ...
  // Create CSS variables directly in the DOM safely
  React.useEffect(() => {
        ...
        if (chartElement) {
          colorConfig.forEach(([key, itemConfig]) => {
            const color =
              ...
              itemConfig.color;
            if (color) {
              (chartElement as HTMLElement).style.setProperty(
                `--color-${key}`,
                color
              );
            }
    ...
    return () => {
      ...
      if (chartElement) {
        colorConfig.forEach(([key]) => {
          (chartElement as HTMLElement).style.removeProperty(`--color-${key}`);
        });
      }
    };
  }, [id, colorConfig]);
